...
public interface EtablissementRepository extends JpaRepository<Etablissement, Long> {
    ...
    @Query(
        value = "select distinct etablissement from Etablissement etablissement left join fetch etablissement.bFPA left join fetch etablissement.nomCFP left join fetch etablissement.nomLycetech",
    ...
    @Query(
        "select distinct etablissement from Etablissement etablissement left join fetch etablissement.bFPA left join fetch etablissement.nomCFP left join fetch etablissement.nomLycetech"
    )
    List<Etablissement> findAllWithToOneRelationships();

    @Query(
        "select etablissement from Etablissement etablissement left join fetch etablissement.bFPA left join fetch etablissement.nomCFP left join fetch etablissement.nomLycetech where etablissement.id =:id"
    )
